    ...
    try {
      ...
      try {
        ...
        userPreferences = body.userPreferences
          ? JSON.parse(body.userPreferences as string)
          : undefined;
      } catch (parseError) {
        return c.json(
          {
      ...
      try {
        if (user && !user.isPremium) {
          const scanCount = await (
            await import("../../database/models/scanHistory")
          ).ScanHistory.countDocuments({ userId: user._id });
          if (scanCount >= 10) {
            return c.json(
              {
                success: false,
                error: getMessage("PREMIUM.SCAN_LIMIT_REACHED"),
              },
              403
            );
          }
      ...
      if (!gameType || !["pokemon", "yugioh", "onepiece"].includes(gameType)) {
        ...
        logger.info(
          `🎯 Detected game type: ${gameType} (${gameClassification.confidence}%)`
        );
      } else {
      ...
      const ocrData = await enhancedOCR.extractCardText(
        imageBuffer,
        gameType as any
      );
      logger.info(
        `🔤 Extracted card name: "${ocrData.extractedText.cardName}"`
      );
      ...
      if (setCodeExtraction.extractedCodes.length > 0) {
        ...
        if (detectedSetCodes.length > 0) {
          ...
          logger.info(
            `🎯 Detected set: ${topSetCode.setCode} - ${topSetCode.setName} (${(
              topSetCode.confidence * 100
            ).toFixed(1)}%)`
          );
        }
      } else {
      ...
      logger.info(
        "🔍 Step 3: Implementing card-number-first search strategy..."
      );
      ...
      const cardNumberResults =
        await cardNumberFuzzySearch.findAllCardsByNumber(
          gameType as any,
          ocrData.extractedText.allText,
      ...
      if (cardNumberResults.matches.length > 0) {
        ...
        if (topCardNumberMatch && topCardNumberMatch.confidence >= 60) {
          logger.info(
            `🎯 Found card number match: ${topCardNumberMatch.card?.name} (${topCardNumberMatch.confidence}%)`
          );

          // Log all cards found for each card number
          Object.entries(cardNumberResults.cardsByNumber).forEach(
            ([cardNumber, cards]) => {
              logger.info(
                `📋 Card number ${cardNumber}: ${cards.length} variants`
              );
          ...
          logger.info(
            `🎯 Card number determined name: "${cardNumberBasedName}"`
          );

          // Override AI correction with card-number-based name if confidence is high enough
          if (topCardNumberMatch.confidence >= 85) {
            logger.info(
              `🎯 High confidence card number (${topCardNumberMatch.confidence}%) - using as authoritative name`
            );
          }
        }
      }
      ...
      if (!isCardNumberSearch) {
        logger.info(
          "🔄 No reliable card numbers found, using OCR + AI correction approach..."
        );
        ...
        cardNumberBasedName =
          aiCorrection.confidence > 70
            ? aiCorrection.correctedName
            : ocrData.extractedText.cardName;
      }
      ...
      if (!isCardNumberSearch) {
        ...
        cardNumberBasedName =
          aiCorrection.confidence > 70
            ? aiCorrection.correctedName
            : ocrData.extractedText.cardName;
        const wasAICorrected =
          cardNumberBasedName !== ocrData.extractedText.cardName;

        if (wasAICorrected) {
          logger.info(
            `🤖 AI corrected: "${ocrData.extractedText.cardName}" → "${cardNumberBasedName}" (${aiCorrection.confidence}%)`
          );
        } else {
          logger.info(
            `🤖 AI confirmed: "${ocrData.extractedText.cardName}" (${aiCorrection.confidence}%)`
          );
        }
      }

      // 🎯 PRIORITY 3: Set-based search (either as primary or fallback)
      let shouldDoSetSearch =
        !isCardNumberSearch ||
        getMatchesLength(searchResults) === 0 ||
        (getMatchesLength(searchResults) > 0 &&
          getTopMatch(searchResults)?.confidence < 85);
      ...
      if (isCardNumberSearch && searchResults) {
        ...
        const exactCardNumberMatches = allMatches.filter(
          (m: any) =>
            m.matchReason &&
            m.matchReason.includes("exact") &&
            m.matchReason.includes("card number")
        );

        if (exactCardNumberMatches.length > 0) {
          shouldDoSetSearch = false;
          logger.info(
            `🎯 Found exact card number matches, skipping set search`
          );
        }
      }
      ...
      if (shouldDoSetSearch) {
        logger.info(
          isCardNumberSearch
            ? "🔄 Adding set-based search for validation..."
            : "🔄 Using set-based search as primary method..."
        );
        ...
        if (detectedSetCodes.length > 0) {
          ...
          logger.info(
            `🎯 Top detected set: ${topSetCode.setCode} - ${
              topSetCode.setName
            } (${setConfidence.toFixed(1)}%)`
          );

          if (setConfidence >= 70) {
            // High confidence - search only in this set using the card-number-based name
            logger.info(
              `🎯 High confidence (${setConfidence.toFixed(
                1
              )}% >= 70%), searching within set: ${topSetCode.setCode}`
            );
            ...
            logger.info(
              `🔍 Set-based search found ${getMatchesLength(
                setSearchResults
              )} matches in ${topSetCode.setCode}`
            );
            ...
            if (isCardNumberSearch && getMatchesLength(searchResults) > 0) {
              ...
              const intersectionMatches = cardNumberMatches.filter(
                (cardMatch) =>
                  setMatches.some(
                    (setMatch) =>
                      setMatch.card._id.toString() ===
                      cardMatch.card._id.toString()
                  )
              );

              if (intersectionMatches.length > 0) {
                logger.info(
                  `🎯 Found ${intersectionMatches.length} cards matching both card number AND set context`
                );
                searchResults.topMatch = intersectionMatches[0];
                searchResults.candidates = [
                  ...intersectionMatches.slice(1),
                  ...cardNumberMatches.filter(
                    (m) => !intersectionMatches.includes(m)
                  ),
                ];
                ...
                searchResults.searchStrategy =
                  "card_number_plus_set_validation";
              } else {
                logger.info(
                  `⚠️ Card number and set results don't overlap, prioritizing card number results`
                );
            ...
            if (getMatchesLength(searchResults) === 0) {
              logger.info(
                "⚠️ Set-based search yielded poor results, expanding to full search..."
              );
              ...
              logger.info(
                `🔍 Fallback search found ${getMatchesLength(
                  searchResults
                )} matches`
              );
            }
            ...
            logger.info(
              `🎯 Moderate set confidence (${setConfidence.toFixed(
                1
              )}% < 70%), doing full search with card-number-based name`
            );
            ...
            const boostedMatches = allMatches
              .map((match: any) => {
                if (
                  match.card?.setCode === topSetCode.setCode ||
                  (match.card?.setName &&
                    match.card.setName.includes(topSetCode.setName))
                ) {
                  return {
                    ...match,
            ...
            logger.info(
              `🔍 Full search with set boosting found ${boostedMatches.length} matches`
            );
          }
        } else {
          // No set detected - normal search with card-number-based name
          logger.info(
            "🔍 No reliable set detected, performing standard search with card-number-based name..."
          );
      ...
      if (isSetBasedSearch && detectedSetCodes.length > 0) {
        ...
        if (setConfidence >= 70) {
          logger.info(
            `🎯 High confidence set detection (${setConfidence.toFixed(
              1
            )}%), limiting visual matching to set: ${topSetCode.setCode}`
          );

          // Get variants from the set-based search results only
          cardVariants = getSearchResultMatches(searchResults).map(
            (match: any) => ({
          ...
          logger.info(
            `🎯 Using ${cardVariants.length} variants from set ${topSetCode.setCode} for visual matching`
          );
        } else {
          ...
          logger.info(
            `🖼️ Found ${cardVariants.length} card variants for visual matching`
          );
        }
      } else {
        // Normal case: get variants from entire database
      ...
      if (cardVariants.length > 0) {
        ...
        visualMatches = visualResults
          .map((visualMatch) => {
            ...
            const matchingTextResult = getSearchResultMatches(
              searchResults
            ).find(
              (textMatch: any) =>
                textMatch.card._id.toString() === visualMatch.cardId
            );

            // Find the card variant data for the name
            const cardVariant = cardVariants.find(
              (variant: any) => variant.cardId === visualMatch.cardId
            );

            // Safety checks for NaN values
            const safeVisualSimilarity = isNaN(visualMatch.similarity)
              ? 0
              : visualMatch.similarity;
            const safeTextConfidence = matchingTextResult
              ? isNaN(matchingTextResult.confidence)
                ? 0
                : matchingTextResult.confidence
              : 0;
        ...
        logger.info(
          `🖼️ Visual matching completed: ${visualMatches.length} visual matches found`
        );
      }
      ...
      let candidates = getSearchResultMatches(searchResults).map(
        (match: any) => ({
          ...
          visualMatch: visualMatches.find(
            (vm) => vm.cardId === match.card._id.toString()
          ),
          // Include game-specific stats
          ...(gameType === "pokemon" && {
            hp: match.card.hp,
      ...
      if (setCodeExtraction.extractedCodes.length > 0 && !isSetBasedSearch) {
        logger.info(
          `🎯 Filtering ${
            candidates.length
          } candidates by detected set codes: [${setCodeExtraction.extractedCodes.join(
            ", "
          )}]`
        );

        const setCodeFilteredCandidates = candidates.filter(
          (candidate: any) => {
            // Get set codes for this candidate card
            const candidateSetCodes =
              setCodeRecognition.extractSetCodeFromCardName(
                candidate.setName || ""
              );

            // Check if any detected OCR set code matches any candidate set code
            const hasSetCodeMatch = setCodeExtraction.extractedCodes.some(
              (ocrCode) =>
                candidateSetCodes.some(
                  (candidateCode) =>
                    setCodeRecognition.compareSetCodes(
                      ocrCode,
                      candidateCode
                    ) ||
                    ...
                    (candidate.setName || "")
                      .toLowerCase()
                      .includes(ocrCode.toLowerCase())
                )
            );

            if (hasSetCodeMatch) {
              logger.info(
                `✅ Set code match found for candidate: ${candidate.name} (${candidate.setName})`
              );
              return true;
            }
        ...
        if (setCodeFilteredCandidates.length > 0) {
          ...
          logger.info(
            `🎯 Set code filtering successful: ${candidates.length} candidates remain`
          );

          // Log remaining candidates
          candidates.forEach((candidate: any, index: number) => {
            logger.info(
              `  ${index + 1}. ${candidate.name} (${candidate.setName}) - ${
          ...
          logger.warn(
            `⚠️ Set code filtering removed all candidates, keeping original ${candidates.length} candidates`
          );
        }
      } else if (isSetBasedSearch) {
        logger.info(
          `🎯 Set-based search already filtered by set, skipping additional filtering`
        );
      }
      ...
      if (visualMatches.length > 0) {
        candidates = candidates
          .map((candidate: any) => {
            ...
            if (visualMatch) {
              ...
              let combinedScore;
              const topSetCode =
                detectedSetCodes.length > 0 ? detectedSetCodes[0] : null;
              const setConfidence = topSetCode
                ? topSetCode.confidence * 100
                : 0;
              ...
              if (isSetBasedSearch && topSetCode && setConfidence >= 70) {
                ...
                logger.info(
                  `🎯 High confidence set mode: ${
                    candidate.name
                  } - Text: ${textScore.toFixed(
                    2
                  )} Visual: ${visualScore.toFixed(
                    2
              ...
              return {
                ...
                matchReason:
                  isSetBasedSearch && topSetCode && setConfidence >= 70
                    ? `Set-based match in ${topSetCode.setCode} (${Math.round(
                        textScore * 100
                      )}% text + ${Math.round(visualScore * 100)}% visual)`
                    : visualScore > 0.8
        ...
        const reorderStrategy =
          isSetBasedSearch && topSetCode && setConfidence >= 70
            ? "set-priority reordering"
        ...
        data: {
          ...
          pipeline: {
            ...
            step5_results: {
              ...
              withVisualData: candidates.filter((c: any) => c.visualMatch)
                .length,
            },
          },
          ...
          requiresSetSelection:
            ...
            candidates
              .slice(0, 3)
              .every((c: any) => parseInt(c.confidence) > 70),
        },
        message:
          candidates.length > 0
        ...
        data: {
          ...
          steps: {
            step1: {
              ...
              description:
                "Automatically detect Pokemon/Yu-Gi-Oh/One Piece using visual analysis",
              features: [
  ...
  private async saveScanHistory(
    ...
    try {
      ...
      await ScanHistory.create({
        ...
        // OCR Results
        extractedText: [
          metadata.extractedCardName || topMatch.name || "Unknown",
        ],
