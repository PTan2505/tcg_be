...
export class DeckController {
  ...
  getUserDecks = async (c: Context) => {
    try {
      ...
      if (!user) {
        return c.json(
          createErrorResponse(MESSAGES.AUTH.AUTHENTICATION_REQUIRED),
          401
        );
      }
      ...
      return c.json(
        createSuccessResponse(result.decks, MESSAGES.DECKS.GET_SUCCESS, {
          total: result.total,
          page: result.page,
          limit: result.limit,
  ...
  createDeck = async (c: Context) => {
    try {
      ...
      return c.json(
        createSuccessResponse(deck, MESSAGES.DECKS.CREATE_SUCCESS),
        201
      );
    } catch (error: any) {
      ...
      const body = createErrorResponse(
        error?.message || MESSAGES.DECKS.CREATE_FAILED
      );
      return new Response(JSON.stringify(body), {
        status: 500,
        headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === "Deck not found or access denied") {
          return c.json(
            createErrorResponse(MESSAGES.DECKS.DECK_NOT_FOUND_OR_ACCESS_DENIED),
            404
          );
        }
        ...
        if (error.message === MESSAGES.VALIDATION.CARD_ID_REQUIRED) {
          return c.json(
            createErrorResponse(MESSAGES.VALIDATION.CARD_ID_REQUIRED),
            400
          );
        }
        ...
        if (error.message === MESSAGES.CARDS.CARD_NOT_FOUND) {
          return c.json(
            createErrorResponse(MESSAGES.CARDS.CARD_NOT_FOUND),
            404
          );
        }
        ...
        if (error.message === MESSAGES.VALIDATION.GAME_TYPE_INVALID) {
          return c.json(
            createErrorResponse(MESSAGES.VALIDATION.GAME_TYPE_INVALID),
            400
          );
        }
  ...
  updateCardInDeck = async (c: Context) => {
    try {
      ...
      try {
        ...
        if (body && typeof body.quantity !== "undefined")
      ...
      const deck = await deckService.updateCardInDeck(
        deckId,
        userId,
        cardId,
        qty
  ...
  removeCardFromDeck = async (c: Context) => {
    try {
      ...
      const deck = await deckService.removeCardFromDeck(
        deckId,
        userId,
        cardId,
        quantity
      ...
      if (error instanceof Error) {
        if (
          error.message === "Deck not found or access denied" ||
          error.message === "Card not found in deck"
        ) {
  ...
  searchDecks = async (c: Context) => {
    try {
      ...
      if (!query) {
        return c.json(
          createErrorResponse(MESSAGES.VALIDATION.SEARCH_QUERY_REQUIRED),
          400
        );
      }
