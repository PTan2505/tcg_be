...
export class PostService {
  ...
  async getFeed(
    ...
    // Mark whether the requesting user has reacted (liked) each post.
    ...
    if (postIds.length > 0) {
      ...
      for (const p of postsAny) {
        p.isSelfReacted = reactedSet.has(p._id?.toString());
      }
    }
  ...
  async toggleReaction(
    ...
    const likesCount = await PostReactionModel.countDocuments({
      post: postId,
      type: "like",
    });
  ...
  private async canUserViewPost(
    ...
    if (
      post.tags &&
      post.tags.some((tag) => tag.toString() === userId.toString())
    )
      return true;
